...
abstract class Util
{
    ...
    public static function arrayToType(array $item)
    {
        ...
        if (isset($item['type'])) {
          if ($item['type'] == 'Video' && strpos($item['duration'], 'PT') == 0) {
            return Type::create('PeerTube'.$item['type'], $item);
          } else {
    ...
    /**
     * Add identifiers for peertube Video type
     *
     * @return [type] [description]
     */
    public static function ptvideo($pt)
    {
      if ($pt['type'] == 'Video' && strpos($pt['duration'], 'PT') == 0) {
        return Type::create('PeerTube'.$item['type'], $item);
      }
    }
    ...
    public static function validateUrl($value)
    {
        ...
        if (is_object($value) && strpos($value->value, '<a') == 0) {
          $value = explode('"', $value->value)[1];
        }
